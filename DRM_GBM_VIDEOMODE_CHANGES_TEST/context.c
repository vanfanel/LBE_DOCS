...
{
    uint32_t width  = dispdata->newmode.hdisplay;
    uint32_t height = dispdata->newmode.vdisplay;
...
void KMSDRM_SwapWindow() {
	...
	if (drmModeSetCrtc(viddata->drm_fd, dispdata->crtc_id, fb_info->fb_id, 0,
				    0, &dispdata->conn->connector_id, 1, &dispdata->newmode)) {
...
void
KMSDRM_SetDisplayMode(int mode_width)
{
    drmModeConnector *conn = dispdata->conn;

    /* Iterate on the mode list of the connector, searching for the mode we need.
       Then recreate the GBM and EGL surfaces via KMSDRM_CreateSurfaces(), which in turn will mark
       the crtc_setup_pending bool so drmModeSetCrtc() is called from the next SwapWindow(). */

    for (int i = 0; i < dispdata->conn->count_modes; i++) {
        dispdata->newmode = conn->modes[i];
        if (dispdata->newmode.hdisplay == mode_width) {
            printf("(INFO) Found %d mode: hdisplay = %d vdisplay = %d, vrefresh = %d \n",
                mode_width, dispdata->newmode.hdisplay, dispdata->newmode.vdisplay, dispdata->newmode.vrefresh);
            break;
        }
    }
