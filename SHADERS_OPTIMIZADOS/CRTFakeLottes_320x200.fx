...
#ifndef EXTRA_MASKS
	#define EXTRA_MASKS 0 // disable these if you need extra registers freed up
#endif
...
float4 PS_CRTFakeLottes(float4 vpos : SV_Position, float2 txcoord : TexCoord) : SV_Target
{
	float4 col;
		...
		col = scanline(pos, res);
	       ...
	       alpha += smoothstep(0.001, 0.0, pos.x); // Suavizado al salir del límite izquierdo
	       alpha += smoothstep(0.999, 1.0, pos.x); // Suavizado al salir del límite derecho
	       alpha += smoothstep(0.001, 0.0, pos.y); // Suavizado al salir del límite inferior
	       alpha += smoothstep(0.999, 1.0, pos.y); // Suavizado al salir del límite superior
