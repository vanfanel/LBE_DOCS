...
static const char* fragment_shader_noeffect =
    "varying mediump vec2 v_texcoord;                       \n"
...
void gles2_init_shaders_orig () {
	...
	shader.program = CreateProgram(vertex_shader_prg, fragment_shader_noeffect);
...
/*void gles2_init_shaders () {
	...
	shader.program = CreateProgram(vertex_shader_prg, fragment_shader_16bit);
...
void gles2_init_texture () {
	...
	// For 32 bpp textures
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _dispvars->texture_width, _dispvars->texture_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL); SHOW_ERROR
	//	
...
void gles2_draw(void *pixels)
{
	...
	// For 32 bpp textures
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, _dispvars->texture_width, _dispvars->texture_height, GL_RGBA, GL_UNSIGNED_BYTE, (uint32_t*) pixels); SHOW_ERROR
