...
void drm_page_flip_handler(int fd, uint frame, uint sec, uint usec, void *data) {
	int *pageflip_pending = (int *)data;
	*pageflip_pending = 0;
}
...
void drmPageFlip(void) {
	...
	eventContext.page_flip_handler = drm_page_flip_handler;

       /* ret = drmModePageFlip(drm.fd, drm.crtc_id, bufs[flip_page].fb,
                                     DRM_MODE_PAGE_FLIP_EVENT, &drm.pageflip_pending);
	*/

        ret = drmModePageFlip(drm.fd, drm.crtc_id, bufs[0].fb,
                                     DRM_MODE_PAGE_FLIP_ASYNC, NULL);

	/*if (ret < 0) {
		printf("failed to commit pageflip: %s\n", strerror(errno));
	}
	else { 
		drm.pageflip_pending = 1;
		flip_page = !(flip_page);
		printf ("commit pageflip SUCCESSFUL!\n");
	}*/
